...
public class TestAction extends SeleneseTestBase{
	...
	public void testCreateNewAction() throws Exception {
		openPage("http://localhost:5500/");
		createAction();
		assertCreation();
	
	}

	private void assertCreation() {
	...
	private void createAction() throws InterruptedException {
	...
	private void openPage(String url) throws InterruptedException {
		selenium.open(url);
		for (int second = 0;; second++) {
			if (second >= 60) fail("timeout");
