...
public class VerifyHelper extends HelperWithWebDriverBase {
		...
		private void findUser(LeadProviderObject leadProvider) {
			click(By.id("TopHeader_ltbnSearch"));
			
			type(By.id("MainBodyPlaceHolder_ContentPlaceHolder1_tbSearch"), leadProvider.userEmail);
			click(By.id("MainBodyPlaceHolder_ContentPlaceHolder1_lblSearch"));
		}
		...
		public void verifyExistNameErrorIsAppear() {
				...
				String errorMessageTextOnPage = findElement(By.cssSelector(".form-error-notif-message")).getText();
		...
		public void verifyLeadProviderAdded(LeadProviderObject leadProvider) {
			
			findUser(leadProvider);
			click(By.id("MainBodyPlaceHolder_ContentPlaceHolder1_gvSearchResult_lbtnEdit_0"));
			verifyLeadProviderFields(leadProvider);
			System.out.println("Assertings for"+leadProvider.userEmail+"passed succesfully");
			
		}
		
		private void verifyLeadProviderFields(LeadProviderObject leadProvider) {
			
			assertFieldValue(leadProvider.LeadProviderAccountName, By.id(MakeFullLink("tbLeadProviderName")));
			assertFieldValue(leadProvider.gmailURL, By.id(MakeFullLink("tbNextGmailUrl")));
			assertFieldValue(manager.getProperty("GroupId"), By.id(MakeFullLink("tbGroupId")));
			
			assertFieldValue(leadProvider.userFirstName, By.id(MakeFullLink("tbFirstName")));
			assertFieldValue(leadProvider.userLastName, By.id(MakeFullLink("tbLastName")));
			assertFieldValue(leadProvider.userEmail, By.id(MakeFullLink("tbEmail")));
